...
export default function ComparisonGrid() {
  ...
  const [previousMonth, setPreviousMonth] = useState<string>(FISCAL_MONTHS[0]);
  ...
  useEffect(() => {
    // Auto-select previous month when current month changes
  ...
  // Available months for "previous" selector (all months before current)
  const currentIdx = FISCAL_MONTHS.indexOf(currentMonth as typeof FISCAL_MONTHS[number]);
  const availablePrevMonths = FISCAL_MONTHS.slice(0, currentIdx);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          <label className="text-sm font-medium text-gray-700 mr-2">비교월:</label>
          <select
            value={previousMonth}
            onChange={(e) => setPreviousMonth(e.target.value)}
            className="border border-gray-300 rounded-md px-3 py-1.5 text-sm"
          >
            {availablePrevMonths.map((m) => (
              <option key={m} value={m}>
                {formatYearMonth(m)}
              </option>
            ))}
